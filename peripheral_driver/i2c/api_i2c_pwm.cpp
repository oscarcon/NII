...
// int
// map_steering (double x, int in_min, int in_max, int out_min, int out_max)
// {
//     //if (x == -90) return 150;
//     //if (x == 90) return 540;
//     double a = abs(x);
//     double toReturn;
//     if (x < 0) {
//         toReturn = (380 - a/90.0*(380-200));
//     }
//     else toReturn = 390 + a/90.0*(580-390);
//     return (int)(round(toReturn));
// }
int map_steering (double x, int in_min, int in_max, int out_min, int out_max)
{ 
    if (/*x >= -90 &&*/ x <= -80) return 180;
    if (/*x <= 90 && */x >= 80) return 580;
...
api_pwm_pca9685_init( PCA9685 *pca9685)
{
    ...
    sleep(1) ;
    int pwm2 = map_steering( 0, MIN_ANGLE, MAX_ANGLE, STEERING_MAX_RIGHT2, STEERING_MAX_LEFT2 );
...
int api_set_STEERING_control( PCA9685 *pca9685,double &theta)
{
    ...
    int pwm2 = map_steering( theta, MIN_ANGLE, MAX_ANGLE, STEERING_MAX_RIGHT2, STEERING_MAX_LEFT2 ); 
    pca9685->setPWM(STEERING_CHANNEL2,0, pwm2);
	int pwm1 = map_steering( -theta, MIN_ANGLE, MAX_ANGLE, STEERING_MAX_RIGHT1, STEERING_MAX_LEFT1 ); 
